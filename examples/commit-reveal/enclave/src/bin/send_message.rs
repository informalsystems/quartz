...
use cw_client::{CliClient, CwClient, GrpcClient};
...
async fn main() {
    ...
    let encrypted_msg: HexBinary = encrypt(&pk_hex.to_sec1_bytes(), serialized_msg.as_bytes()).unwrap().into();

    // Prepare cosmwasm message
    // Set pubkey to user's pubkey
    let reveal_msg: ExecuteMsg = ExecuteMsg::Ping(Ping {
        pubkey: HexBinary::from_hex("026452a47ff13ef0aefcaf79b4e68389c55759abaa644166e99c1b9bfc904597d4").unwrap(),
    ...
    let output = cw_client
        .tx_execute(
            &AccountId::from_str("neutron1u0ehv853npcmu9m4jexampykq6yeuf6nlnxpvm5m8w73g2vrv9wqyx0mdp").unwrap(),
            chain_id,
            2000000,
            "admin",
            json!(reveal_msg),
