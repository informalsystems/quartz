...
pub mod core_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + Send,
    {
        ...
        pub async fn health(
            ...
            let codec = tonic::codec::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static("/quartz.Core/Health");
            let mut req = request.into_request();
            req.extensions_mut().insert(GrpcMethod::new("quartz.Core", "Health"));
            self.inner.unary(req, path, codec).await
        }
    }
}
        ...
        B::Error: Into<StdError> + Send + 'static,
    {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                ...
                "/quartz.Core/Health" => {
                    #[allow(non_camel_case_types)]
                    struct HealthSvc<T: Core>(pub Arc<T>);
                    impl<T: Core> tonic::server::UnaryService<super::HealthRequest>
                    for HealthSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as Core>::health(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
