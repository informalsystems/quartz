...
use scripts::{types::{WasmdTxResponse, Log}, utils::{wasmaddr_to_id, block_tx_commit}};
...
use mtcs_overdraft::{msg::{InstantiateMsg, QueryMsg, QueryResp, Cw4ExecuteMsg, Member}, AddHook};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let group_deploy_output: WasmdTxResponse = serde_json::from_str(
        &wasmd_client.deploy(
    ...
    let mc_wasm = contracts_path.join("overdraft/target/wasm32-unknown-unknown/release/mtcs_overdraft.wasm");
    // TODO: uncertain about the path -> string conversion
    let mc_deploy_output: WasmdTxResponse = serde_json::from_str(
        &wasmd_client.deploy(
            &ChainId::from_str("testing")?, 
            String::from("wasm14qdftsfk6fwn40l0xmruga08xlczl4g05npy70"), 
    ...
    let init_msg = InstantiateMsg {
        owner: Addr::unchecked("wasm14qdftsfk6fwn40l0xmruga08xlczl4g05npy70"), memberships_code_id
    };
