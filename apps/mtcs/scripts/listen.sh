    ...
    (echo "$SUBSCRIBE"; cat) | websocat $WSURL | while read msg; do 
        ...
        cd "$ROOT/apps/mtcs"
        TRUSTED_HASH=$(cat trusted.hash)
        TRUSTED_HEIGHT=$(cat trusted.height)

        cd $ROOT/utils/tm-prover
        PROOF_FILE="light-client-proof.json"
        ...
        POP=$(cat $PROOF_FILE)
        REQUEST_MSG=$(jq --argjson msg "$REQUEST" '. + {msg: $msg}' <<< "$POP")
        RUN_CLEARING_MSG=$(jq -nc --arg message "$REQUEST_MSG" '$ARGS.named')

        echo "... executing mtcs"
        ATTESTED_MSG=$(grpcurl -plaintext -import-path "$DIR_PROTO" -proto mtcs.proto -d "$RUN_CLEARING_MSG" "127.0.0.1:$QUARTZ_PORT" mtcs.Clearing/Run | jq -c '.message | fromjson')
        QUOTE=$(echo "$ATTESTED_MSG" | jq -c '.attestation')
        MSG=$(echo "$ATTESTED_MSG" | jq -c '.msg')

        if [ -n "$MOCK_SGX" ]; then
            echo "... running in MOCK_SGX mode"
            EXECUTE=$(jq -nc --argjson submit_setoffs "$(jq -nc --argjson msg "$MSG" --argjson attestation "$QUOTE" '$ARGS.named')" '$ARGS.named')
        else
            ...
            EXECUTE=$(jq -nc --argjson submit_setoffs "$(jq -nc --argjson msg "$MSG" --argjson attestation \
