...
pub mod clearing_server {
    ...
    #[async_trait]
    pub trait Clearing: std::marker::Send + std::marker::Sync + 'static {
        async fn run(
            &self,
    ...
    impl<T> Clone for ClearingServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "mtcs.Clearing";
    impl<T> tonic::server::NamedService for ClearingServer<T> {
