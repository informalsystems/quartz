...
impl Attestor for DcapAttestor {
    ...
    fn attestation(&self, user_data: impl HasUserData) -> Result<Self::Attestation, Self::Error> {
        ...
        fn collateral(
            ...
            let mut root_crl = include_bytes!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../../contracts/tee-ra/data/root_crl.der"
            ))
            .to_vec();
            root_crl.push(0);
            ...
            let root_cert = include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../../contracts/tee-ra/data/root_ca.pem"
            ));
            let tcb_cert = include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../../contracts/tee-ra/data/tcb_signer.pem"
            ));
            ...
            const QE_IDENTITY_JSON: &str = include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../../contracts/tee-ra/data/qe_identity.json"
            ));
