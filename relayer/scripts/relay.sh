...
if [ -n "$MOCK_SGX" ]; then
    ...
    # Fetch collateral data from Intel Attestation Service
   COLLATERAL=$(curl -s "https://global.acccache.azure.net/sgx/certification/v3/" | jq -c '.')
    echo $COLLATERAL
...
#     # Fetch collateral data from Intel Attestation Service
#     COLLATERAL=$(curl -s "https://api.trustedservices.intel.com/sgx/certification/v3/pckcert" | jq -c '.')

#     case "$REQUEST" in
#     "Instantiate")
#         jq -nc --argjson msg "$MSG" --argjson "attestation" \
#             "$(jq -nc --argjson report "$(jq -nc --argjson report "$REPORT" --arg reportsig "$REPORTSIG" --argjson collateral "$COLLATERAL" '$ARGS.named')" '$ARGS.named')" \
#             '$ARGS.named'
#         ;;
